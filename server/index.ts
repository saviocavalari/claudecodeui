...
// File Tree API Routes (protected). The two guards are mounted here rather than
// inside the module so authorization stays in the composition root: members may
// only read/write files of projects granted to them, and walking the host
// filesystem outside any project stays admin-only.
app.use('/api/file-tree/projects/:projectId', authenticateToken, enforceProjectIdAccess);
app.use('/api/file-tree/browse-filesystem', authenticateToken, requireAdmin);
