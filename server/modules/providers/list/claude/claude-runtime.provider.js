...
function mapCliOptionsToSDK(options = {}) {
  ...
  // `providerEnv` layers the active account's profile on top (multi-account
  // support), so it is applied after the server's own environment.
  sdkOptions.env = {
    ...process.env,
...
async function queryClaudeSDK(command, options = {}, ws, context) {
  ...
  // Credentials of the account this user runs under (multi-account support);
  // replaced below once the account service resolves the active profile.
  ...
  try {
    ...
    // the app session id while a brand-new run has no provider id yet, so
    // chat.abort always has something to address from the first moment.
    if (sessionKey()) {
