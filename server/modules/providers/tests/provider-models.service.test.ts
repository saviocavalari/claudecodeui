...
// This fork caches Claude like every other provider: the Claude adapter already
// queries the SDK live (with a timeout and a fixed-catalog fallback), so the
// service-level cache keeps multi-account runs from hitting the SDK per request.
