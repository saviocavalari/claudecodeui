...
export const projectsDb = {
    createProjectPath(projectPath: string, customProjectName: string | null = null): CreateProjectPathResult {
        ...
        INSERT INTO projects (project_id, project_path, custom_project_name, isArchived)
            ...
            RETURNING project_id, project_path, custom_project_name, isStarred, isArchived, emoji, folder
    ...
    getProjectPath(projectPath: string): ProjectRepositoryRow | null {
        ...
        const normalizedProjectPath = normalizeProjectPath(projectPath);
        const row = db.prepare(`
            SELECT project_id, project_path, custom_project_name, isStarred, isArchived, emoji, folder
            FROM projects
            WHERE project_path = ?
    ...
    updateProjectEmojiById(projectId: string, emoji: string | null): void {
        const db = getConnection();
        db.prepare(`
            UPDATE projects
            SET emoji = ?
    ...
    updateProjectFolderById(projectId: string, folder: string | null): void {
        const db = getConnection();
        db.prepare(`
            UPDATE projects
            SET folder = ?
            WHERE project_id = ?
        `).run(folder, projectId);
    },
